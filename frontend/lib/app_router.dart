...
import 'presentation/tenant/join/tenant_rental_join_screen.dart';
...
class AppRouter {
  static String _homeForRole(String? role) {
    switch ((role ?? '').toUpperCase()) {
      ...
      case 'TENANT':
        return SessionStore.instance.requiresRentalJoin
            ? '/tenant/join-rental'
            : '/tenant/dashboard';
      default:
  ...
  static final router = GoRouter(
    ...
    redirect: (context, state) {
      final isLoggedIn = SessionStore.instance.isLoggedIn;
      final role = (SessionStore.instance.role ?? '').toUpperCase();
      final requiresRentalJoin = SessionStore.instance.requiresRentalJoin;
      final location = state.matchedLocation;
      final isLoginOrRegister = location == '/login' || location == '/register';
      final isPublicAuthRoute =
          isLoginOrRegister || location == '/reset-password';
      final isTenantJoinRoute = location == '/tenant/join-rental';

      if (!isLoggedIn && !isPublicAuthRoute) return '/login';
      if (isLoggedIn && isLoginOrRegister) return _homeForRole(role);
      ...
      if (isLoggedIn && role == 'TENANT' && requiresRentalJoin) {
        if (!isTenantJoinRoute) {
          return '/tenant/join-rental';
        }
      }
      if (isLoggedIn &&
          role == 'TENANT' &&
    ...
    routes: [
      ...
      GoRoute(
        path: '/tenant/join-rental',
        builder: (context, state) => const TenantRentalJoinScreen(),
      ),
      GoRoute(
